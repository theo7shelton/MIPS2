...
subprogram_1:						# hexToDec
	...
	li $s1, 0				
     ...
     checkChar:
	li $t1, 0x0000003A
	slt $t2, $a1, $t1				# Sets $t2 if t0 <3Ah
	li $t1, 0x0000002F				
	slt $t3, $t1, $a1				# Sets $t3 if $t0 > 2Fh
	...
	li $t1, 0x00000047
	slt $t2, $a1, $t1				# Sets $t2 if t0 < 47
	li $t1, 0x00000040				
	slt $t3, $t1, $a1				# Sets $t3 if $t0 > 40
	...
	li $t1, 0x00000067
	slt $t2, $a1, $t1				# Sets $t2 if t0 < 67
	li $t1, 0x00000060				
	slt $t3, $t1, $a1				# Sets $t3 if $t0 > 60
	and $s0, $t2, $t3				# Sets $s0 if byte is in range of a-f ASCII 
	addu $s1, $s1, $s0				# If falls in the range, increments $s1
...
subprogram_2: 						# convertString
	...
	sb $t6, 1($sp)					# Return if end of string
	sw $s6, 2($sp)
